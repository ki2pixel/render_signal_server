...
- **Execution Flow**:
  ...
  2. Read only necessary files with `read_file` / `search`, then immediately fix with `edit`.
...
- **Updating**: Utilisez l'outil `fast_edit_block` avec `path` et `old_text`/`new_text` pour synchroniser l'état du projet en fin de tâche.
...
- **Authorized Tools**: `task-master-ai`, `sequentialthinking_tools`, `fast_edit_block`, `fast_read_multiple_files`, `json_query_jsonpath`, `json_query_search_keys`, `render-signal-mcp`, `redis-signal-mcp-server`.
...
### 2.8 Render.com Operations (render-signal-mcp)
- **Deployment Tasks**: Use `create_web_service`, `update_web_service`, `create_postgres` for infrastructure provisioning
- **Monitoring**: Use `get_metrics`, `list_deploys`, `get_service` for deployment verification and troubleshooting
- **Environment Management**: Use `update_environment_variables` for configuration updates in production
