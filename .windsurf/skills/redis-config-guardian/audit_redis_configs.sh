...
python -m scripts.check_config_store --keys magic_link_tokens polling_config processing_prefs webhook_config --raw

echo "\n[info] Comparing with debug/*.json fallbacks"
for key in magic_link_tokens polling_config processing_prefs webhook_config; do
  echo "--- $key ---"
