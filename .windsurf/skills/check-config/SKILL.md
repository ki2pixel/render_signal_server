...
2. **Inspection rapide**
   ...
   ```
   - Inspecte `processing_prefs`, `webhook_config`, `polling_config`.
...
4. **API dashboard (optionnel)**
   - `POST /api/verify_config_store` via client authentifié avec `{ "keys": ["polling_config"], "includeRawJson": true }` pour recouper avec l'UI.
5. **Remédiation**
