...
```mermaid
flowchart TD
A([BEGIN]) --> B[Audit Structurel et Métrique - Cartographie, Volumétrie, Complexité]
B --> C[Diagnostic Triangulé - Comparer Intention vs Réalité vs Existant]
C --> D[Sélection du Standard de Rédaction selon le type de dossier]
D --> E[Proposition de Mise à Jour - Générer plan de modification]
E --> F[Application et Finalisation - Exécuter les changements]
F --> G([END])
```

## 🚨 Protocoles Critiques

1. **Outils autorisés** : L'usage de `execute_command` est **strictement limité** aux commandes d'audit : `tree`, `cloc`, `radon`, `ls`.
...
1. **Cartographie (Filtre Bruit)** :
   - `execute_command "tree -L 2 -I '__pycache__|venv|node_modules|.git|htmlcov|debug|deployment|memory-bank|.shrimp_task_manager'"`
   - *But* : Visualiser l'architecture modulaire (`email_processing`, `services`, `routes`, `background`).

2. **Volumétrie (Code Source)** :
   - `execute_command "cloc . --exclude-dir=tests,docs,venv,node_modules,htmlcov,debug,deployment,memory-bank,.shrimp_task_manager --exclude-ext=json,txt,log --md"`
...
3. **Complexité Cyclomatique (Python Core)** :
   - `execute_command "radon cc . -a -nc --exclude='tests/*,venv/*,htmlcov/*,docs/*,deployment/*,setup.py,.shrimp_task_manager/*'"`
...
4. **Exploration Scripts Approfondie** :
   - `execute_command "tree -L 4 scripts/ background/ routes/ services/ -I '__pycache__|venv|node_modules|.git|htmlcov|debug|deployment|memory-bank|.shrimp_task_manager'"`
...
5. **Volumétrie Scripts Spécialisés** :
   - `execute_command "cloc scripts/ background/ routes/ services/ --exclude-dir=tests,.shrimp_task_manager --exclude-ext=json,txt,log --md"`
...
6. **Complexité Scripts Utilitaires** :
   - `execute_command "radon cc scripts/ background/ routes/ services/ -a -nc --exclude='tests/*,.shrimp_task_manager/*'"`
...
2. **Mise à jour Memory Bank** :
   - Respecter strictement `.clinerules/memorybankprotocol.md` : `fast_read_file` pour les lectures sélectives, puis utiliser les outils d'édition du workspace uniquement si une mise à jour de la Memory Bank est explicitement demandée.

### Sous-protocole Rédaction — Application de documentation/SKILL.md

1. **Point d'entrée** : Après validation du plan (Étape 4), charger `.agents/skills/documentation/SKILL.md` via `read_text_file`.
