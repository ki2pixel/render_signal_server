...
- Le helper `./.agents/skills/run-tests/run_tests.sh` est un helper legacy minimal qui exécute `pytest --cov=.`. Préférez `./run_tests.sh` à la racine pour bénéficier des options réelles du projet.