...
- **Execution Flow**:
  ...
  2. Read only necessary files with `read_file` / `search`, then immediately fix with `edit_file`.
...
- **Execution Flow**:
  ...
  2. Read related files and make changes incrementally with `edit_file` / `multi_replace_file_content`.
...
- **Reading**: Utilisez l'outil `fast_read_file` avec le paramètre `path` pour les chemins absolus.
...
- **Updating**: Utilisez l'outil `edit_file` avec `path` et `old_text`/`new_text` pour synchroniser l'état du projet en fin de tâche.
- **Writing**: Utilisez l'outil `fast_write_file` avec `path` et `content` uniquement pour l'initialisation ou l'archivage.
- **List**: Utilisez l'outil `fast_list_directory` avec `path` pour lister les fichiers disponibles.
- **Diagnostic**: Utilisez l'outil `fast_get_directory_tree` pour vérifier la structure des répertoires.
...
### 2.7 Authorized Tools and Policies
- **Authorized Tools**: `shrimp-task-manager`, `sequentialthinking_tools`, `edit_file`, `fast_read_multiple_files`, `json_query_query_json`, `json_query_search_keys`, `redis-signal-mcp-server`.
- **Complexity Lock**: If the request involves a major feature or entire project, the AI is obligated to request execution of `enhance_complex`.
- **JSON Management**: Prohibition on fully reading (`read_file`) large JSON files (manifest, i18n). Use `json-query` to extract only relevant nodes before modification.

### 2.8 Render.com Operations (render-deployment-manager)
- **Deployment Tasks**: Use local scripts via the `render-deployment-manager` skill for infrastructure provisioning
- **Monitoring**: Use local REST endpoints for deployment verification and troubleshooting
- **Environment Management**: Update environment variables through the Render API via the manager
- **Security**: Always validate workspace selection before operations; never expose credentials

### 2.9 Redis Operations (redis-signal-mcp-server)
- **Configuration Access**: Use `json_get`, `hgetall` for reading application configs from Redis
- **Data Updates**: Use `json_set`, `hset` for updating configuration data
- **Key Management**: Use `scan_keys`, `delete`, `expire` for key discovery and lifecycle management
- **Performance**: Prefer direct MCP operations over shell scripts for real-time config inspections
