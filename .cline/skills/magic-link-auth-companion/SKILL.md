...
7. **Revocation (urgence)**
   - Utiliser le helper `python ./.cline/skills/magic-link-auth-companion/revoke_magic_links.py --all` ou `python ./.cline/skills/magic-link-auth-companion/revoke_magic_links.py --token <uuid>` pour révoquer en masse ou individuellement.
