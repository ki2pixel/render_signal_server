...
if TRIGGER_PAGE_USER_ENV and TRIGGER_PAGE_PASSWORD_ENV:
    ...
    app.logger.info(f"CFG AUTH: Trigger page user '{TRIGGER_PAGE_USER_ENV}' configured.")
else:
    app.logger.warning("CFG AUTH: TRIGGER_PAGE_USER or TRIGGER_PAGE_PASSWORD not set. HTTP Basic Auth for trigger page will NOT be actively enforced.")
...
if POLLING_ACTIVE_DAYS_RAW:
    try:
        ...
        app.logger.warning(f"CFG POLL: Invalid POLLING_ACTIVE_DAYS. Using default Mon-Fri.")
...
if POLLING_TIMEZONE_STR.upper() != "UTC":
    if ZoneInfo:
        ...
        app.logger.warning(f"CFG POLL: 'zoneinfo' not available. Using UTC. '{POLLING_TIMEZONE_STR}' ignored.")
        POLLING_TIMEZONE_STR = "UTC"
if TZ_FOR_POLLING is None: 
    TZ_FOR_POLLING = timezone.utc
app.logger.info(f"CFG POLL: Timezone for polling: {POLLING_TIMEZONE_STR}. Active: {POLLING_ACTIVE_START_HOUR:02d}-{POLLING_ACTIVE_END_HOUR:02d}, Days: {POLLING_ACTIVE_DAYS}")
...
app.logger.info(f"CFG PATH: Signal directory for ephemeral files (if Redis fails/unavailable): {SIGNAL_DIR.resolve()}")


# --- Configuration Redis ---
REDIS_URL = os.environ.get('REDIS_URL')
redis_client = None
LOCALTUNNEL_URL_REDIS_KEY = "current_localtunnel_url_v1" # Added _v1 for potential future changes

if REDIS_AVAILABLE and REDIS_URL:
    try:
        redis_client = redis.from_url(REDIS_URL, socket_connect_timeout=5, socket_timeout=5) # Added timeouts
        redis_client.ping()
        ...
        app.logger.error(f"CFG REDIS: Could not connect to Redis at {REDIS_URL.split('@')[-1] if '@' in REDIS_URL else REDIS_URL}. Error: {e_redis}. Will use file fallback for Localtunnel URL.")
        redis_client = None
    except Exception as e_redis_other: # Catch other potential redis client errors
        app.logger.error(f"CFG REDIS: An unexpected error occurred during Redis initialization: {e_redis_other}. Will use file fallback.")
        redis_client = None
elif REDIS_AVAILABLE and not REDIS_URL:
    app.logger.warning("CFG REDIS: REDIS_URL not set. Redis client library is available, but Redis will not be used. Using file fallback for Localtunnel URL.")
    redis_client = None # Explicitly set to None
else: # REDIS_AVAILABLE is False
    app.logger.warning("CFG REDIS: 'redis' Python library not installed. Redis will not be used. Using file fallback for Localtunnel URL.")
    redis_client = None # Explicitly set to None
...
if ONEDRIVE_CLIENT_ID and ONEDRIVE_CLIENT_SECRET:
    app.logger.info(f"CFG MSAL: Initializing MSAL. ClientID: '{ONEDRIVE_CLIENT_ID[:5]}...', Authority: {ONEDRIVE_AUTHORITY}")
    ...
    app.logger.warning("CFG MSAL: OneDrive Client ID/Secret missing. OneDrive & Email Polling disabled.")
...
if SENDER_LIST_FOR_POLLING: app.logger.info(f"CFG POLL: Monitoring emails from: {SENDER_LIST_FOR_POLLING}")
...
if not EXPECTED_API_TOKEN: app.logger.warning("CFG TOKEN: PROCESS_API_TOKEN not set. Make.com API endpoints insecure.")
else: app.logger.info(f"CFG TOKEN: PROCESS_API_TOKEN configured: '{EXPECTED_API_TOKEN[:5]}...'")
...
else: app.logger.info(f"CFG TOKEN: REGISTER_LOCAL_URL_TOKEN configured: '{REGISTER_LOCAL_URL_TOKEN[:5]}...'")
...
def get_onedrive_access_token():
    ...
    if not ONEDRIVE_REFRESH_TOKEN:
        app.logger.error("MSAL: OneDrive refresh token missing (ONEDRIVE_REFRESH_TOKEN env var).")
        return None
    ...
    if "access_token" in token_result:
        ...
        if token_result.get("refresh_token") and token_result.get("refresh_token") != ONEDRIVE_REFRESH_TOKEN:
            app.logger.warning("MSAL: New refresh token issued. IMPORTANT: Update ONEDRIVE_REFRESH_TOKEN env var: "
...
def ensure_onedrive_folder(access_token, subfolder_name=None, parent_folder_id=None):
    ...
    try:
        ...
        if children:
            ...
            app.logger.info(f"OD_UTIL: Folder '{clean_target_folder_name}' found (ID: {folder_id}).")
            return folder_id
        app.logger.info(f"OD_UTIL: Folder '{clean_target_folder_name}' not found. Creating.")
        create_url = f"https://graph.microsoft.com/v1.0/me/drive/{parent_path_segment}/children"
        payload = {"name": clean_target_folder_name, "folder": {}, "@microsoft.graph.conflictBehavior": "rename"}
        ...
        app.logger.info(f"OD_UTIL: Folder '{clean_target_folder_name}' created (ID: {created_folder_id}).")
        return created_folder_id
    except requests.exceptions.RequestException as e:
        app.logger.error(f"OD_UTIL: Error ensuring folder '{clean_target_folder_name}': {e}")
        if hasattr(e, 'response') and e.response is not None: app.logger.error(f"OD_UTIL: API Status: {e.response.status_code}, Body: {e.response.text[:500]}")
        return None

def get_processed_items_from_onedrive_file(job_id_prefix, token, folder_id, filename):
    if not token or not folder_id:
        app.logger.error(f"DEDUP_ITEMS [{job_id_prefix}]: Token/FolderID missing for '{filename}'.")
        return set()
    ...
    app.logger.debug(f"DEDUP_ITEMS [{job_id}]: Reading '{filename}'.")
    try:
        ...
        if response.status_code == 200:
            ...
            app.logger.info(f"DEDUP_ITEMS [{job_id}]: File '{filename}' not found.")
        else: response.raise_for_status()
    except requests.exceptions.RequestException as e:
        app.logger.error(f"DEDUP_ITEMS [{job_id}]: Error downloading '{filename}': {e}")
        if hasattr(e, 'response') and e.response is not None: app.logger.error(f"DEDUP_ITEMS [{job_id}]: API Response: {e.response.status_code} - {e.response.text[:200]}")
    return items
...
def add_item_to_onedrive_file(job_id_prefix, token, folder_id, filename, item_to_add):
    if not all([token, folder_id, filename, item_to_add]):
        app.logger.error(f"DEDUP_ITEMS [{job_id_prefix}]: Missing params for '{filename}'.")
        return False
    job_id = f"{job_id_prefix}_{time.time_ns() % 100000}"
    ...
    if item_to_add in current_items:
        app.logger.info(f"DEDUP_ITEMS [{job_id}]: Item '{item_to_add}' already in '{filename}'.")
        return True
    ...
    app.logger.debug(f"DEDUP_ITEMS [{job_id}]: Updating '{filename}' with '{item_to_add}'. Items: {len(current_items)}.")
    try:
        response = requests.put(upload_url, headers=headers, data=content_to_upload.encode('utf-8'), timeout=60)
        response.raise_for_status()
        app.logger.info(f"DEDUP_ITEMS [{job_id}]: File '{filename}' updated with '{item_to_add}'. Items: {len(current_items)}.")
        return True
    except requests.exceptions.RequestException as e:
        app.logger.error(f"DEDUP_ITEMS [{job_id}]: Error uploading to '{filename}': {e}")
        if hasattr(e, 'response') and e.response is not None: app.logger.error(f"DEDUP_ITEMS [{job_id}]: API Response: {e.response.status_code} - {e.response.text[:200]}")
        return False
...
def check_new_emails_and_trigger_make_webhook():
    ...
    if not SENDER_LIST_FOR_POLLING: app.logger.warning("POLLER: SENDER_LIST_FOR_POLLING empty."); return 0
    if not MAKE_SCENARIO_WEBHOOK_URL: app.logger.error("POLLER: MAKE_SCENARIO_WEBHOOK_URL not set."); return 0
    if not msal_app: app.logger.error("POLLER: MSAL app not configured."); return 0
    token = get_onedrive_access_token()
    if not token: app.logger.error("POLLER: Failed to get OneDrive token."); return 0
    onedrive_app_folder_id = ensure_onedrive_folder(token)
    if not onedrive_app_folder_id: app.logger.error("POLLER: Failed to ensure OneDrive folder."); return 0
    processed_email_ids = get_processed_items_from_onedrive_file("WH_TRIG", token, onedrive_app_folder_id, PROCESSED_WEBHOOK_TRIGGERS_ONEDRIVE_FILENAME)
    ...
    try:
        ...
        app.logger.info(f"POLLER: Found {len(emails)} unread email(s) matching criteria (before dedup).")
        for mail in emails:
            mail_id = mail['id']
            ...
            if mail_id in processed_email_ids:
                app.logger.debug(f"POLLER: Email ID {mail_id} (Subj: '{mail_subject[:30]}...') already processed. Skipping.")
                continue
            app.logger.info(f"POLLER: New email: ID={mail_id}, Subj='{mail_subject[:50]}...'. Triggering Make.")
            payload_for_make = {"microsoft_graph_email_id": mail_id, "subject": mail_subject,
                                "receivedDateTime": mail.get("receivedDateTime"), "sender_address": sender_address,
            ...
            try:
                ...
                if webhook_response.status_code == 200 and "accepted" in webhook_response.text.lower():
                    app.logger.info(f"POLLER: Make webhook OK for email {mail_id}. Resp: {webhook_response.status_code} - {webhook_response.text}")
                    if add_item_to_onedrive_file("WH_TRIG_ADD", token, onedrive_app_folder_id, PROCESSED_WEBHOOK_TRIGGERS_ONEDRIVE_FILENAME, mail_id):
                        ...
                        if not mark_email_as_read(token, mail_id): app.logger.warning(f"POLLER: Failed to mark email {mail_id} as read.")
                    else: app.logger.error(f"POLLER: CRITICAL - Failed to log email ID {mail_id}. Email NOT marked read.")
                else: app.logger.error(f"POLLER: Make webhook FAILED for email {mail_id}. Status: {webhook_response.status_code}, Resp: {webhook_response.text[:200]}")
            except requests.exceptions.RequestException as e_webhook: app.logger.error(f"POLLER: Exception during Make webhook for email {mail_id}: {e_webhook}")
        ...
        if hasattr(e_graph, 'response') and e_graph.response is not None: app.logger.error(f"POLLER: API Resp: {e_graph.response.status_code} - {e_graph.response.text[:500]}")
        return 0
...
def background_email_poller():
    app.logger.info(f"BG_POLLER: Email polling thread started. TZ: {POLLING_TIMEZONE_STR}.")
    consecutive_error_count = 0
    MAX_CONSECUTIVE_ERRORS = 5
    while True:
        try:
            now_in_configured_tz = datetime.now(TZ_FOR_POLLING)
            current_hour, current_weekday = now_in_configured_tz.hour, now_in_configured_tz.weekday()
            ...
            log_sched = (f"Day:{current_weekday}[{POLLING_ACTIVE_DAYS}], "
            ...
            if is_active_day and is_active_time:
                ...
                if not all([ONEDRIVE_CLIENT_ID, ONEDRIVE_CLIENT_SECRET, ONEDRIVE_REFRESH_TOKEN, 
                    ...
                    app.logger.warning("BG_POLLER: Essential polling config incomplete. Waiting 60s.")
                ...
                app.logger.info(f"BG_POLLER: Outside active period ({log_sched}). Sleeping.")
            ...
            app.logger.error(f"BG_POLLER: Unhandled error in loop (Err #{consecutive_error_count}): {e}", exc_info=True)
            if consecutive_error_count >= MAX_CONSECUTIVE_ERRORS:
                app.logger.critical(f"BG_POLLER: Max errors ({MAX_CONSECUTIVE_ERRORS}). Stopping thread.")
                break
    ...
    try:
        if redis_client: # Try Redis first
            if new_lt_url:
                ...
                app.logger.info(f"API_REG_LT_URL: Localtunnel URL registered in Redis: '{new_lt_url}'")
            ...
            if new_lt_url:
                ...
                app.logger.info(f"API_REG_LT_URL: Localtunnel URL (fallback file) registered: '{new_lt_url}'")
            else: # Unregister
                if LOCALTUNNEL_URL_FILE.exists(): LOCALTUNNEL_URL_FILE.unlink()
                app.logger.info(f"API_REG_LT_URL: Localtunnel URL (fallback file) unregistered/cleared.")
        return jsonify({"status": "success", "message": "Localtunnel URL processed."}), 200
    ...
    try:
        if redis_client: # Try Redis first
            ...
            if url_bytes:
                ...
                app.logger.info(f"API_GET_LT_URL: Fetched Localtunnel URL from Redis: '{lt_url}'")
            else:
                app.logger.info(f"API_GET_LT_URL: Localtunnel URL not found in Redis (key: {LOCALTUNNEL_URL_REDIS_KEY}).")
        else: # Fallback to ephemeral file storage if Redis was never available
            app.logger.warning(f"API_GET_LT_URL: Redis unavailable, trying fallback file for Localtunnel URL.")
            if LOCALTUNNEL_URL_FILE.exists():
                ...
                if lt_url_file_content: # Ensure file is not empty
                    ...
                    app.logger.info(f"API_GET_LT_URL: Fetched Localtunnel URL from fallback file: '{lt_url}'")
                else:
        ...
        if LOCALTUNNEL_URL_FILE.exists():
            try:
                ...
                if lt_url:
                    app.logger.info(f"API_GET_LT_URL: Fetched (after Redis error) from fallback file: '{lt_url}'")
                    return jsonify({"status": "success", "localtunnel_url": lt_url}), 200
        ...
        return jsonify({"status": "error", "message": "Server error (Redis op failed) and fallback unavailable."}), 503 # Service Unavailable
    except Exception as e:
...
def api_log_processed_url():
    ...
    if not EXPECTED_API_TOKEN: return jsonify({"s": "err", "m": "Server API token not set."}), 500
    if api_token != EXPECTED_API_TOKEN: return jsonify({"s": "err", "m": "Unauthorized."}), 401
    try: data = request.get_json()
    except Exception: return jsonify({"s":"err","m":"JSON format error"}),400
    if not data or 'dropbox_url' not in data: return jsonify({"s": "err", "m": "'dropbox_url' required."}), 400
    ...
    if not msal_app: return jsonify({"s":"err", "m":"OneDrive not configured"}), 503
    token = get_onedrive_access_token()
    if not token: return jsonify({"s": "err", "m": "OneDrive token failed."}), 500
    onedrive_app_folder_id = ensure_onedrive_folder(token)
    if not onedrive_app_folder_id: return jsonify({"s": "err", "m": "OneDrive folder access failed."}), 500
    ...
    else: return jsonify({"s": "err", "m": f"Failed to update log."}), 500
...
def check_local_workflow_trigger():
    ...
    if TRIGGER_SIGNAL_FILE.exists(): # Still using file for this signal
        try:
            ...
            app.logger.info(f"LOCAL_CHECK_API: Signal read from file, deleted. Payload: {payload_from_file}")
        except Exception as e:
...
def serve_trigger_page_main():
    ...
    try:
        ...
        app.logger.error(f"ROOT_UI: CRITICAL - 'trigger_page.html' not found in {app.root_path}.")
        return "Error: Main page content not found.", 404
    except Exception as e:
        app.logger.error(f"ROOT_UI: Error serving 'trigger_page.html': {e}", exc_info=True)
...
def get_local_status_proxied():
    ...
    try: # Fetch URL using the same logic as /api/get_local_downloader_url but without token check here (already auth'd by Basic)
        ...
        if not localtunnel_url and not redis_client: # If redis_client is None or URL not found in Redis
            if LOCALTUNNEL_URL_FILE.exists():
                with open(LOCALTUNNEL_URL_FILE, "r") as f: 
                    file_content = f.read().strip()
                    if file_content: localtunnel_url = file_content # Assign if not empty
    ...
    try:
        ...
        else: app.logger.warning(f"PROXY_STATUS: No INTERNAL_WORKER_COMMS_TOKEN_ENV. Calling {target_url} unauth'd.")
        ...
        return jsonify({
            "overall_status_text": local_data.get("overall_status_code", "état inconnu"), 
            "status_text": local_data.get("status_text_detail", local_data.get("last_updated_utc", "")),
            "progress_current": local_data.get("progress_current", 0), 
            "progress_total": local_data.get("progress_total", 0),
            "current_step_name": local_data.get("current_step_name", "N/A"), 
        ...
        if status_code == 401:
            app.logger.error(f"PROXY_STATUS: 401 Unauth from worker {target_url}. Check INTERNAL_WORKER_COMMS_TOKEN_ENV.")
            err_msg, detail_msg = "Erreur Authentification Interne", "Auth vers worker local échouée."
        return jsonify({"overall_status_text": err_msg, "status_text": detail_msg}), status_code
...
def trigger_local_workflow_authed():
    ...
    if not payload: payload = {"command": "start_manual_generic"}
    elif not isinstance(payload, dict): payload = {"command": "start_manual_generic", "original_payload": payload}
...
if __name__ == '__main__':
    ...
    if not users: app.logger.warning("MAIN_APP: HTTP Basic Auth for UI NOT configured (TRIGGER_PAGE_USER/PASSWORD env vars likely not set).")
    if not EXPECTED_API_TOKEN: app.logger.critical("MAIN_APP: PROCESS_API_TOKEN not set. Make.com endpoints INSECURE.")
    if not REGISTER_LOCAL_URL_TOKEN: app.logger.warning("MAIN_APP: REGISTER_LOCAL_URL_TOKEN not set. Local worker registration endpoint INSECURE.")
    if not REMOTE_UI_ACCESS_TOKEN_ENV and users: app.logger.info("MAIN_APP: REMOTE_UI_ACCESS_TOKEN for /api/get_local_status NOT set; ui_token check skipped if Basic Auth'd.")
