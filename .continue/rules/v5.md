...
- **Reading**: Utilisez l'outil `mcp0_fast_read_file` avec le paramètre `path` pour les chemins absolus.
...
- **Updating**: Utilisez l'outil `mcp0_fast_edit_block` avec `path` et `old_text`/`new_text` pour synchroniser l'état du projet en fin de tâche.
- **Writing**: Utilisez l'outil `mcp0_fast_write_file` avec `path` et `content` uniquement pour l'initialisation ou l'archivage.
- **List**: Utilisez l'outil `mcp0_fast_list_directory` avec `path` pour lister les fichiers disponibles.
- **Diagnostic**: Utilisez l'outil `mcp0_fast_get_directory_tree` pour vérifier la structure des répertoires.
