...
@login_required
def migrate_configs_to_redis_endpoint() -> Response | tuple[Response, int]:
...
@login_required
def verify_config_store() -> Response | tuple[Response, int]:
...
@login_required
def deploy_application() -> Response | tuple[Response, int]:
    ...
    try:
        ...
        # 1) Hook Render
        if render_config.get("deploy_hook_url"):
            resp_tuple = _deploy_via_hook(render_config["deploy_hook_url"])
            if resp_tuple:
                return resp_tuple
                
        # 2) API Render
        if render_config.get("api_key") and render_config.get("service_id"):
            return _deploy_via_api(render_config)
            
        # 3) Fallback local
        return _deploy_via_fallback()
        
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500


def _deploy_via_hook(hook_url: str) -> tuple[Response, int] | None:
    try:
        ...
        masked = hook_url.split("?key=")[0] + "?key=***" if "?key=" in hook_url else hook_url
        user_id = getattr(current_user, "id", "unknown")
        current_app.logger.info("ADMIN: Deploy via Render Deploy Hook requested by '%s' (url=%s)", user_id, masked)

        resp = requests.get(hook_url, timeout=15)
        if resp.status_code in (200, 201, 202, 204):
            current_app.logger.info("ADMIN: Deploy hook accepted (http=%s)", resp.status_code)
            return jsonify({
                "success": True,
                "message": "Déploiement Render déclenché via Deploy Hook. Consultez le dashboard Render.",
            }), 200
            
        current_app.logger.warning("ADMIN: Deploy hook returned non-success http=%s; will try alternative method.", resp.status_code)
    except Exception as e:
...
def _deploy_via_api(render_config: dict) -> tuple[Response, int]:
    ...
    resp = requests.post(url, json={"clearCache": render_config.get("clear_cache")}, headers=headers, timeout=20)
    data = resp.json() if resp.status_code in (200, 201, 202) else {"raw": resp.text[:400]}

    if resp.status_code in (200, 201, 202):
        deploy_id = data.get("id") or data.get("deployId")
        current_app.logger.info("ADMIN: Render deploy accepted (id=%s, status=%s, http=%s)", deploy_id, data.get("status", "queued"), resp.status_code)
        return jsonify({
            "success": True,
...
def _deploy_via_fallback() -> tuple[Response, int]:
    default_cmd = (
...
@login_required
def check_emails_and_download() -> Response | tuple[Response, int]:
    try:
