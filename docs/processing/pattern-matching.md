...
```python
# services/ingress_service.py (MÉTHODE ACTUELLE GMAIL PUSH)
# IngressService._get_detector_and_time
ms_res = pattern_matching.check_media_solution_pattern(subject, body, tz_for_polling, self._logger)
des_res = pattern_matching.check_desabo_conditions(subject, body, self._logger)
```
