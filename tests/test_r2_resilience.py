...
def test_r2_worker_failure_does_not_break_webhook_send_exception(monkeypatch):
    ...
    monkeypatch.setattr(DeduplicationService.get_instance(), 'mark_email_processed', lambda eid: True)
    monkeypatch.setattr('email_processing.imap_client.mark_email_as_read_imap', lambda *a, **k: None, raising=False)
    monkeypatch.setattr(DeduplicationService.get_instance(), 'generate_subject_group_id', lambda s: "g-r2-1", raising=False)
...
def test_r2_worker_failure_does_not_break_webhook_send_none(monkeypatch):
    ...
    monkeypatch.setattr(DeduplicationService.get_instance(), 'mark_email_processed', lambda eid: True)
    monkeypatch.setattr('email_processing.imap_client.mark_email_as_read_imap', lambda *a, **k: None, raising=False)
    monkeypatch.setattr(DeduplicationService.get_instance(), 'generate_subject_group_id', lambda s: "g-r2-2", raising=False)
    monkeypatch.setattr(DeduplicationService.get_instance(), 'is_subject_group_processed', lambda gid: False, raising=False)
    monkeypatch.setattr(ar, "_rate_limit_allow_send", lambda: True, raising=False)
    monkeypatch.setattr(ar, "_record_send_event", lambda: None, raising=False)
    monkeypatch.setattr(ar, "_append_webhook_log", lambda e: None, raising=False)
