...
@pytest.mark.unit
def test_check_trigger_absent(tmp_path, authenticated_flask_client):
    from routes import api_utility as mod

    with patch.object(mod, 'TRIGGER_SIGNAL_FILE', tmp_path / 'trigger.json'):
        r = authenticated_flask_client.get('/api/check_trigger')
        assert r.status_code == 200
        data = r.get_json()
...
@pytest.mark.unit
def test_check_trigger_present_valid_json(tmp_path, authenticated_flask_client):
    from routes import api_utility as mod
...
@pytest.mark.unit
def test_check_trigger_present_malformed_json(tmp_path, authenticated_flask_client):
    from routes import api_utility as mod
