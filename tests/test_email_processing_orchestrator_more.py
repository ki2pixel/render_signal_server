...
def test_outside_window_recadrage_is_marked_and_not_sent(monkeypatch):
    ...
    monkeypatch.setattr('email_processing.imap_client.mark_email_as_read_imap', lambda l, m, n: calls.__setitem__('read', calls['read'] + 1), raising=False)
    from services.deduplication_service import DeduplicationService
    DeduplicationService.reset_instance()
    monkeypatch.setattr(DeduplicationService.get_instance(), 'mark_email_processed', lambda eid: calls.__setitem__('marked', calls['marked'] + 1))
    from services.deduplication_service import DeduplicationService
    DeduplicationService.reset_instance()
    monkeypatch.setattr(DeduplicationService.get_instance(), 'is_email_processed', lambda eid: False)
    monkeypatch.setattr(DeduplicationService.get_instance(), 'generate_subject_group_id', lambda s: 'g1')
    monkeypatch.setattr(DeduplicationService.get_instance(), 'is_subject_group_processed', lambda gid: False)
...
def test_outside_window_neutral_skips_without_marking(monkeypatch):
    ...
    monkeypatch.setattr(DeduplicationService.get_instance(), 'mark_email_processed', lambda eid: calls.__setitem__('marked', calls['marked'] + 1))
    monkeypatch.setattr(DeduplicationService.get_instance(), 'is_email_processed', lambda eid: False)
    monkeypatch.setattr(DeduplicationService.get_instance(), 'generate_subject_group_id', lambda s: 'g3', raising=False)
    monkeypatch.setattr(DeduplicationService.get_instance(), 'is_subject_group_processed', lambda gid: False, raising=False)
    monkeypatch.setattr(ar, '_rate_limit_allow_send', lambda: True, raising=False)
    monkeypatch.setattr(ar, '_record_send_event', lambda: None, raising=False)
    monkeypatch.setattr(ar, '_append_webhook_log', lambda e: None, raising=False)
