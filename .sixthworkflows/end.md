...
4. **Mise à jour finale de progress.md**  
   - Utiliser `edit_file` ou `multi_edit` pour déplacer les tâches vers la section "Terminé" avec un nouveau timestamp et s'assurer que "En cours" est défini sur "Aucune tâche active".

5. **Réinitialisation d'activeContext.md**  
  - Employer `edit_file` ou `multi_edit` pour remettre le contenu à l'état neutre décrit dans les règles.
